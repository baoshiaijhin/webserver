...
static const int max_connect_timeout=75;
...
void toEpoll(tinyrpc::FdEvent::ptr fd_event, int events) {
	...
	if (events & tinyrpc::IOEvent::WRITE) {
		LOG_DEBUG("fd:[%d], register write event to epoll",fd_event->getFd());
...
ssize_t writev_hook(int fd, const struct iovec* iov, size_t iovCnt) {
	// DebugLog << "this is hook write";
  if (tinyrpc::Coroutine::IsMainCoroutine()) {
    // DebugLog << "hook disable, call sys write func";
  ...
  ssize_t n = g_sys_writev_fun(fd, iov, iovCnt);
  if (n > 0) {
    return n;
  }
...
int connect_hook(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
  ...
  tinyrpc::TimerEvent::ptr event = std::make_shared<tinyrpc::TimerEvent>(max_connect_timeout, false, timeout_cb);
